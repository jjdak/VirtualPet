...
readonly BUNDLE_ID="${BUNDLE_ID:-com.yourcompany.VirtualPet}"
readonly DEVICE_NAME_VALUE="${DEVICE_NAME:-iPhone 17}"
readonly DEVICE_UDID_OVERRIDE="${DEVICE_UDID:-}"
readonly DERIVED_DATA="${DERIVED_DATA:-${PROJECT_ROOT}/.runtime/iOSPreviewDerivedData}"
readonly SCREENSHOT="${SCREENSHOT:-${PROJECT_ROOT}/.runtime/ios-preview.png}"
...
[[ -n "$DEVICE_UDID_VALUE" ]] || {
  echo "no available iOS Simulator named '$DEVICE_NAME_VALUE'; set DEVICE_NAME or DEVICE_UDID" >&2
  exit 1
}
...
DEVELOPER_DIR="$DEVELOPER_DIR_VALUE" xcodebuild \
  ...
  -scheme "$SCHEME" \
  -destination "platform=iOS Simulator,id=$DEVICE_UDID_VALUE" \
  -derivedDataPath "$DERIVED_DATA" \
  ENABLE_USER_SCRIPT_SANDBOXING=NO \
  CODE_SIGNING_ALLOWED=NO \
  build

app_path="$DERIVED_DATA/Build/Products/Debug-iphonesimulator/VirtualPet.app"
[[ -d "$app_path" ]] || {
